...
`캐시`란 자주 사용되는 데이터를 임시 저장하는 저장소라 보면 된다.  
웹뿐만 아니라 메모리,데이터베이스,CPU 등 여러곳에서 사용된다.  
일반적으로 캐시의 장점은 자주 사용하는 연산의 데이터, 프로그램이나 애플리케이션이 자주 접근하는 데이터등을
캐시에 저장해 빠른게 접근을 할 수 있습니다.  
이로 인해 메인 서버나 메모리에 부하를 감소하고 효과적인 캐시 사용은 전반적인 시스템 향상에 도움을 줍니다.  

__HTTP에서 캐시란?__  

위와 같이 웹환경에서 캐시를 사용하면 변경되지 않은 데이터인 이미지 혹은 JS, CSS와 같은 파일들은 로컬캐시에서 불러와서 서버에 부하를 줄일 수 있고, 사용자가 데이터를 빠르게 로드하여 빠른 서비스를 제공하고 그에 따라 네트워크 비용도 절감할 수 있습니다.  

### 캐시의 종류
사설(private) 캐시 :  사설 캐시는 웹브라우저에 저장되는 캐시로 다른 사용자는 접근할 수 없는 캐시  

공유(shared) 캐시 : 다수의 사용자가 재사용할 수 있도록 응답을 저장하는 캐시로 웹 브라우저와 서버 사이에서 동작하는 캐시  
-> 프록시(Proxy) 캐시와 관리형(Managed) 캐시가 공유캐시에 속한다.  
### 캐시 관련 헤더  

Cache-Control = HTTP에서 캐시 메커니즘을 지정하기 위해 사용되는 헤더  
Last-Modified , ETag = 캐시 리소스를 검증하기 위한 헤더
If-Modified-Since , If-None-Match = 조건부 요청 헤더

- __캐시 제어__
    - Cache-Control: max-age=time(s)
        ex) Cache-Control: max-age=3600(단위 : 초)  
        웹 브라우저가 특정 리소스에 최초로 요청했을 시 서버는 위와 같이 Cache-Control 헤더가 포함된 응답  
        표현방식은 fresh는 위 max-age 시간을 초과안한 것 , stale은 시간을 초과한 것  
        fresh한 상태에서는 해당 리소스를 캐시 저장소에서 사용자에게 제공함  
    ...
    - Cache-Control: must-revalidate  
        ...
        원 서버 접근 실패시 반드시 오류가 발생해야함 - 상태코드 : 504(Gateway Timeout)  
        must-revalidate는 캐시 유효 시간이라면 캐시를 사용  
    - Pragma: no-cache (HTTP 1.0 하위 호환)  
    - expires : 정확한 만료 시간 (해당 시간 까지 유지함)  
        ex) expires: Mon, 02 Jan 2023 00:00:00 GMT  
        expires는 Cache-Control: max-age가 있으면 우선순위에서 밀린다.(무시됨)  

- __캐시 유효성 검증 및 조건부 요청__  

    `캐시의 유효 기간`이 지난다면?
    -> 이전과 다르지 않은 데이터를 그대로 응답 or 새롭게 갱신된 데이터를 응답  
    이때, `전자의 경우`에는 `의미 없는 트래픽을 낭비`한 것  
    실제 원본 데이터가 수정 되었을 때만 리소스를 내려 받는것이 트래픽낭비를 안하는 바람직 방법이다.  
    그 방법으로 시간으로 검증하는 방법, 식별자를 통한 검증 방법이 있다. 

    - Last-Modified / If-Modified-Since : 시간으로 검증하는 헤더  
        `최초 요청 시`: 응답에 Last-Modified: (리소스가 수정된 마지막 시간) 포함  
        `두번째 요청 시`: 요청헤더에 If-Modified-Since: (Last-Modified 헤더로 받은 시간 값) 포함  
        __리소스 미변경 시__  
        304 Not Modified 상태코드와 함께 바디가 없는 상태(`트래픽 낭비를 방지`)로 응답한다.  
        __리소스 변경 시__  
        200 OK 상태코드와 변경된 리스소의 바디를 포함하여 응답한다.   
    
    - ETag / If-None-Match : 식별자로 검증하는 헤더  
        `최초 요청 시`: 응답에 ETag를 포함  
        해당 ETag값을 브라우저는 저장한다  
        ETag는 요청한 리소스의 현재 버전에 대한 식별자이다.  
        `두번째 요청 시`:  If-None-Match: (저장해둔 ETag 값) 을 포함한 요청  
        __리소스 미변경 시__  
        304 Not Modified 상태코드와 함께 바디가 없는 상태(`트래픽 낭비를 방지`)로 응답한다.  
        __리소스 변경 시__  
        200 OK 상태코드와 변경된 리스소의 바디를 포함하여 응답한다.  
...
- __프록시 캐시__
    프록시 캐시서버는 아까 위에 설명한 shared cache에 해당한다.  
    우리가 만약 유튜브를 본다 했을때 유튜브 본사와 통신하여 영상을 보게 된다면 매우 느리지 않을까?  
    그래서 우리는 항상 유튜브의 프록시 캐시 서버를 경유 하는데,  
    이때 경유하는 프록시 서버에 한국에서 많이 보는 영상이 캐싱되어 있다면 동영상을 빠르게 볼 수 있고,
    만약 내가 최초로 보는(한국에 있는 캐싱서버에 없는 영상 데이터)라면 동영상이 로딩되는데 오래 걸릴것이다.
    이렇게 클라이언트에서 사용하고 저장하는 캐시를 private 캐시, 프록시 캐시를 public 캐시라고 한다.  
    - Cache-Control: public  
        응답이 public 캐시(프록시 서버)에 저장되어도 됨  
